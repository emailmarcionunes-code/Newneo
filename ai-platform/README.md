...
Browser tests start the production build on `127.0.0.1:3100`, exercise the first three launch steps, and run axe checks and capture screenshots at 1180px, 768px and 390px. Results are written to `test-results/` and `playwright-report/`. CI installs Chromium and runs the same checks.
...
The Model stage provides a continuation boundary; the later implementation must supply Model & Runtime, Governance, Evaluate, Deploy and Success. No live evaluation result or production deployment is simulated by the new Launch Guide.
