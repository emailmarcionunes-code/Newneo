...
import {readRetrievalEvaluations,createRetrievalSuite,runRetrievalEvaluation} from '@/server/retrieval-evaluations';
...
async function handle(request:Request,write=false){
 ...
 if(write&&!['create','run'].includes(String(body?.action)))throw new RegistryError(400,'Invalid action.');
 const scope={organizationId:session.organizationId,workspaceId:session.workspaceId};
 const result=await withVerifiedIdentity(session,(db,actor)=>write?(body?.action==='create'?createRetrievalSuite(db,scope,actor,body):runRetrievalEvaluation(db,scope,actor,body)):readRetrievalEvaluations(db,scope,Number(new URL(request.url).searchParams.get('page')??0)));
 return Response.json(result,{headers:noStore});
 }catch(e){return Response.json({error:e instanceof RegistryError?e.message:'Retrieval evaluations are temporarily unavailable.'},{status:e instanceof RegistryError?e.status:503,headers:noStore})}
}
