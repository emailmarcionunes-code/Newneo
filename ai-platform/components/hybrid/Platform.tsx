...
export function Settings() {
  const names = ['Organization', 'Users & Roles', 'API Keys', 'Integrations'];
  const [tab, setTab] = useState('Organization');
  const [org, setOrg] = useState('Acme Corp');
  const [message, setMessage] = useState('');
  const [alerts, setAlerts] = useState([true, true, true, false, false]);
  return (
    <div className="surfacePage hybridPage">
      <PageTitle
        title="Organization Settings"
        description="Manage organization profile, access, credentials and integrations."
      />
        ...
        {tab === 'Organization' ? (
          <>
            <section className="panel">
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  setMessage('Organization profile saved in this preview.');
                }}
              >
            ...
            <div className="hybridSplit equal">
              <section className="panel">
                ...
                {[
                  ...
                  <label className="usageLine" key={k}>
                    {k} · {n} / {max}
                    <progress max={Number(max)} value={Number(n)} />
                  </label>
                ))}
              </section>
              <section className="panel">
                ...
                {[
                  ...
                  <label className="notificationChoice" key={v}>
                    <input
                      type="checkbox"
                      role="switch"
                      checked={alerts[i]}
                      onChange={(e) =>
                        setAlerts((a) =>
                          a.map((x, j) => (j === i ? e.target.checked : x)),
                        )
                      }
                    />
                    {v}
                ...
                <p className="hybridDataNote">
                  Preview preferences do not send notifications.
                </p>
              </section>
            </div>
          </>
        ) : tab === 'Users & Roles' ? (
          <GovernanceEditor initialTab="Users & roles" />
        ) : tab === 'API Keys' ? (
          <section className="panel">
            <h2>API & Webhooks</h2>
            <p>No API credentials have been configured for this workspace.</p>
            <Button
              onClick={() =>
                setMessage(
                  'API key preview: key creation will be available after enterprise identity is configured.',
                )
              }
            >
              Create key preview
          ...
          <>
            <section className="panel">
              ...
              <Link href="/knowledge">Manage knowledge sources →</Link>
              <p>
                <Link href="/tools">Manage Tools & MCP →</Link>
              </p>
              <p>
                <Link href="/models">Approved model endpoints →</Link>
              </p>
            </section>
            <AccountSettings />
          </>
