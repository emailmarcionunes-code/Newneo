...
export default function SkillInventoryLayout({metrics,action,note,view,onView,children}:{metrics:[string,string,string?][];action:ReactNode;note?:ReactNode;view:string;onView:(view:string)=>void;children:ReactNode}) {
 return <div className="surfacePage hybridPage skillsWorkspace"><PageTitle title="Skills" description="Reusable enterprise capabilities shared across your AI agents.">{action}</PageTitle><Metrics items={metrics}/>{note && <p className="skillsDemoNote">{note}</p>}<Tabs names={skillViews} current={view} onChange={onView}/><Panel names={skillViews} current={view}>{children}</Panel></div>;
}
