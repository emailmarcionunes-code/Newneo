'use client';
import { useDemoAccess, CapabilityNotice } from './DemoExperience';
  ...
  return (
    <div className="surfacePage hybridPage operationalEditor">
      ...
      <Panel names={tabs} current={tab}>
        <CapabilityNotice cap={tab === 'Approvals' ? 'approve' : 'admin'}>
          {tab === 'Users & roles' && (
            <>
                    ...
                    <article className="agentCard" key={r.id}>
                      ...
                      {(['Active', 'Rejected'] as const).map((next) => (
                        <Button
                          ...
                          disabled={!note.trim() || !can('approve')}
                          onClick={() => {
