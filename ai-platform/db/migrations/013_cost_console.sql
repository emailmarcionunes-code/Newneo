-- The control-plane schema remains private. Only this narrowly scoped read is exposed.
CREATE TABLE newneo_cost.console_owners (
 identity_id uuid PRIMARY KEY REFERENCES newneo.identities(id),
 granted_at timestamptz NOT NULL DEFAULT now()
);
CREATE FUNCTION newneo.read_cost_console() RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,pg_temp AS $$
...
BEGIN
 ...
 SELECT coalesce(jsonb_agg(to_jsonb(e)),'[]'::jsonb) INTO events FROM (SELECT id,kind,created_at,delivered_at,attempts,next_attempt_at FROM newneo_cost.events ORDER BY created_at DESC,id LIMIT 25) e;
 ...
 'drivers',drivers,'events',events,'approvals',approvals,'pendingNotifications',(SELECT count(*) FROM newneo_cost.events WHERE delivered_at IS NULL));
END $$;
REVOKE ALL ON FUNCTION newneo.read_cost_console() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION newneo.read_cost_console() TO newneo_app;
