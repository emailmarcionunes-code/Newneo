CREATE FUNCTION newneo.require_platform_owner() RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,pg_temp AS $$
DECLARE actor uuid;
BEGIN
 ...
 IF actor IS NULL THEN RAISE EXCEPTION 'Platform-owner access is required.' USING ERRCODE='42501'; END IF;
 RETURN actor;
END $$;
REVOKE ALL ON FUNCTION newneo.require_platform_owner() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION newneo.require_platform_owner() TO newneo_app;
...
BEGIN
 actor:=newneo.require_platform_owner();
