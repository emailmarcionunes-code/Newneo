...
Skills opens in Pipeline view: horizontal lanes by maturity with compact, linked Skill cards and contained row scrolling. Shared filters affect Pipeline and List; selected filters survive switching between those views. Matrix and Intelligence (including most-used and portfolio opportunities) are separate tabs, avoiding a vertically stacked presentation. Maturity lanes visualize existing classification only; no drag-and-drop promotion or implicit readiness change is introduced.
